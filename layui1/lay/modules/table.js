...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'view', 'conf'], function (exports) {
  ...
  Class.prototype.renderToolbar = function () {
    ...
    //by cch
    if (that.config.title != undefined) {
      elemToolTemp.append('<span>' + that.config.title + '</span>')
    }
    //添加工具栏右侧面板
    var layout = {
      filter: {
  ...
  Class.prototype.renderData = function (res, curr, count, sort) {
    var that = this
      ...
      , totalRowData = res[options.response.totalRowName] == undefined ? res[options.response.totalRowName] : res[options.response.totalRowName][0] //合计行数据  by cch
      , trs = []
      , trs_fixed = []
      , trs_fixed_r = []
  ...
  Class.prototype.events = function () {
    ...
    that.layBody.on('change', '.' + ELEM_EDIT, function () {
      ...
      var cacheVal = table.cache[that.key][index][field];
