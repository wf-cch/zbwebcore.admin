...
layui
  .extend({
    ...
    //dropdown1: 'lay/modules/dropdown'
  })
  .define(
    ...
    function (exports) {
      ...
      self.createRequestParams = function (params) {
        ...
        var defaultParams = {
          ...
          success: function (res) {
            if (res.code == 403) {
              layer.msg('登录超时,3秒后返回登录页面...', { shade: 0.01 });
              setTimeout(function () {
                layui.admin.logout()
              }, 3000);
            }
          ...
          error: function (res) {
            layer.close(loadind)
            layer.msg('请检查你的网络连接!', { shade: 0.01 });
            setTimeout(function () {
              layui.admin.logout()
            }, 3000);
          ...
          , complete: function () {
             layer.close(loadind)
            if ($.isFunction(complete)) complete(loadind)
        ...
        if (conf.tokenName) {
          ...
          if (token) {
            defaultParams.headers[conf.tokenName] = token
          }
        }
      ...
      self.settoolbtn = function (title, url) {
        if (title == '条件查询' || title == '首页') return;
        if (title.indexOf('*') >= 0) return
        var htmlall = "", id = "";
        var ida = '[lay-title="' + title + '"]';
        if (url != undefined && url != '') {
          var urlobj = url.split('/');
          id = urlobj[urlobj.length - 1];
        } else {
          var rout = layui.router();
          id = rout.path[rout.path.length - 1];
        }

        var htmltop = '<blockquote class="layui-elem-quote"><div class="layui-btn-group">';
        var htmlend = '</div></blockquote>';
        self.request($.extend({ type: 'post' }, {
          url: '/qxgl/getBtn', data: { menumc: title }, success: function (res) {
            if (res.Data != "") {
              var reshtml = res.Data.replace(/lay-active/g, 'lay-active-' + id);
              htmlall = htmltop + reshtml + htmlend;
              $(ida).prepend(htmlall);
